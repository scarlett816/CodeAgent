...
class AgentLoop:
    ...
    def run(self, user_input: str):
        ...
        # Planner
        #
        plan = self.planner.make_plan(user_input)
        console.print()
        console.rule("[bold cyan]Planner")
        console.print(f"[green]Task Type:[/green] {plan.task_type}")
        console.print(f"[green]Description:[/green] {plan.description}")
        console.print("[green]Suggested Steps:[/green]")
...
Please use these steps as guidance.
You may adjust them according to tool results.
"""
            #
            # 构造 messages
            #
            ...
            if action is None:
                ...
                self.memory.add_tool(
                    "JSON解析失败，请重新输出合法 JSON。"
                )
                continue
            #
            # FINAL
            #
            if self.planner.is_final(action):
            ...
            if (
                ...
                # Memory 只保存源码
                #
                self.memory.add_tool(result["content"])
                continue
            #
            # search_code
            #
            if action["action"] == "search_code":
                ...
                self.memory.add_tool(str(result))
                continue
            #
            # list_files
            #
        ...
        #
        console.print()
        console.print("[red]达到最大循环次数[/red]")
